            ...
            <v-row>
              ...
              <v-col class="d-flex">
                <v-btn
                  ...
                  class="signup_btn"
                >
                  <v-icon>mdi-calendar</v-icon>予約内容を変更する
                </v-btn>
              </v-col>
            </v-row>
          </v-form>
...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch("getReserve", {
      ...
      if(this.roles_id == 1){
        this.name = this.reserved_teacher.teacher_name
      }
      if(this.roles_id == 2){
        this.name = this.reserved_teacher.student_name
      }
      if(this.roles_id == 1){
        this.name = this.reserved_teacher.teacher_name
      }
      this.date = this.reserved_teacher.start_time.slice(0,10);
      this.start_time = this.reserved_teacher.start_time.slice(-8);
      this.end_time = this.reserved_teacher.end_time.slice(-8);
    });
  },
  methods: {
    ...mapActions(["drawer", "close", "reserve", "getTeacher", "getReserve"]),
    clickReserve() {
      if (this.$refs.form.validate()) {
        this.$store.dispatch("updateReserve", {
          reserve_id: this.$route.params.id,
          current_student_id: this.current_student_id,
          teacher_id: this.reserved_teacher.teacher_id,
